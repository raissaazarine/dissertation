"""Qualitative example figure for Gaussian noise: same patch (idx=50) used
for the blur/contrast examples, at 7 representative noise levels, plus
ground truth and a genuinely fresh 'clean' prediction (gaussian_sweep.py
computes clean_pred as its own forward pass, separate from noise_std=0.00's,
so pix2pix's dropout-driven gap between the two is visible here too).
"""
import pathlib
import numpy as np
...
OUT_PNG = pathlib.Path('./gaussian_example_patch50.png')
...
for j, s in enumerate(STDS):
    pred = np.asarray(Image.open(PRED_ROOT / std_dirname(s) / f'{IDX}.tif')) / 255.0
