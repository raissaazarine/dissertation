...
pairwise_difference_heatmap(): one heatmap per faithfulness metric (Deletion
AUC, Insertion AUC, Sensitivity, Entropy), each a 3x3 matrix of mean paired
differences between XAI attribution methods (row - column), starred by
paired t-test p-value. Reuses the full-val-set CSV already produced by the
pix2pix xai_evaluation.py run -- no re-computation needed. Fills the one gap
in xai_evaluation/full_val/ that didn't already have a diffusion-matching
figure (xai_method_comparison.png <-> summary_metrics.png and
xai_pairwise_difference_heatmap_single.png already match 1:1).
"""
import pathlib
import numpy as np
import pandas as pd
...
OUT_PNG = pathlib.Path('./xai_evaluation/full_val/xai_pairwise_difference_heatmap.png')
...
fig.suptitle('Pix2Pix -- Pairwise mean difference between XAI attribution methods\n' + subtitle,
             fontsize=12)
plt.tight_layout()
