"""
MC-Dropout uncertainty vs. prediction error over the full val set.
Port of pix2pix.ipynb's "FULL VALIDATION SET perturbation-based XAI evaluation" /
uncertainty cell (dropout kept active at inference via training=True, N_RUNS
stochastic forward passes per image -> per-pixel predictive variance).
Slow: ~7000+ images x N_RUNS forward passes.
"""
import matplotlib
matplotlib.use('Agg')
...
PATH = pathlib.Path('./datasets') / dataset_name
